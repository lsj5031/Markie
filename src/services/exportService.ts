 ...
 */
const paginateContent = (originalContent: HTMLElement, size: ExportSize): PageContent[] => {
  const pages: PageContent[] = [];
  
  // Get all top-level markdown elements
  const elements = originalContent.children;
  if (elements.length === 0) {
    return [{ container: originalContent.cloneNode(true) as HTMLElement, content: originalContent.cloneNode(true) as HTMLElement }];
  }
...
const prepareCloneForExport = (
  ...
  Object.assign(clone.style, {
    ...
    transform: 'none',
    margin: '0',
    padding: getComputedStyle(originalElement).getPropertyValue('--theme-padding') || '4rem',
    display: 'block',
    visibility: 'visible',
...
export const exportPreview = async (
  ...
  const originalContent = element.querySelector('#preview-content') as HTMLElement;
  ...
  // Single PNG export - entire content as one image
    ...
    try {
      // For PNG, we want to capture the entire content in one image
      // Use a longer height to accommodate all content
      const baseWidth = 1240;
      const exportWidth = baseWidth;
      const exportHeight = size === 'A4' ? Math.round(baseWidth * 1.4142) : baseWidth;
      
      // Create a wrapper that allows content to flow naturally
      ...
      // Add the entire original content to wrapper (not paginated)
      const contentClone = originalContent.cloneNode(true) as HTMLElement;
      wrapper.appendChild(contentClone);
      
      // Apply the theme styles from original element
      const clone = element.cloneNode(true) as HTMLElement;
      clone.innerHTML = '';
      clone.appendChild(wrapper);
      
      // Use auto height to fit all content
      Object.assign(clone.style, {
        width: `${exportWidth}px`,
        height: 'auto',
